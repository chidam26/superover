...
let updateButtonText = () => {
    ...
    if (team1.runs.length == 6 && team2.runs.length == 6) {button.remove();
    //check if match drawn
    result.textContent = team1.score === team2.score ? `Match Drawn` : `${team1.score>team2.score? team1.name: team2.name} Wins`;}
    else {
...
var calculateScore = (runs) => {
    return runs.map(num => {return num == 'W'? 0: num;
